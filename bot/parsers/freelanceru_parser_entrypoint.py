...
# -*- coding: utf-8 -*-
"""
Freelance.ru RSS parser:
- Опрашивает публичный RSS: https://freelance.ru/rss/projects/
...
from bot.storage import load_last_freelanceru_project, save_last_freelanceru_project
...
log = logging.getLogger("freelanceru_parser")

FREELANCERU_RSS_URL = os.getenv("FREELANCERU_RSS_URL", "https://freelance.ru/projects/rss/")
FREELANCERU_PARSER_ENABLED = os.getenv("FREELANCERU_PARSER_ENABLED", "true").lower() in ("1", "true")

if not FREELANCERU_PARSER_ENABLED:
    log.info("Freelanceru parser disabled by env, exiting")
    import sys
    sys.exit(0)
...
M_LOOP    = Counter("freelanceru_loop_total",    "Parser main loops")
M_ERRS    = Counter("freelanceru_errors_total",  "Unhandled errors")
M_SENT    = Counter("freelanceru_cards_sent_total", "Cards sent")
M_SKIPPED = Counter("freelanceru_skipped_total", "Projects skipped")
G_LAST_ID = Gauge("freelanceru_last_id",         "Last processed Freelanceru project id")
...
def _safe_metrics() -> None:
    ...
    if _METRICS_STARTED:
        return
    try:
        port = int(os.getenv("FREELANCERU_METRICS_PORT", "8006"))
        if port > 0:
...
async def _fetch_rss(url: str, timeout: float = 30.0) -> List[Dict[str, Any]]:
    ...
    for entry in feed.entries:
        ...
        cards.append({
            "id":          pid,
            "title":       title or f"Freelance.ru #{pid}",
...
async def run() -> None:
    ...
    if admin_chat:
        test_card = {
            "title": "Freelanceru Parser: Self-test",
            "description": f"Service started, RSS: {FREELANCERU_RSS_URL}",
            "url": FREELANCERU_RSS_URL,
            "source": "freelanceru",
        }
        try:
    ...
    last_id = int(load_last_freelanceru_project() or 0)
    G_LAST_ID.set(last_id)
    log.info("Start Freelanceru loop, url=%s, last_id=%s", FREELANCERU_RSS_URL, last_id)

    while not stop_flag.is_set():
        M_LOOP.inc()
        try:
            cards = await _fetch_rss(FREELANCERU_RSS_URL)
            log.info("Freelanceru RSS: %d projects fetched", len(cards))
            ...
            for card in cards:
                ...
                for tg_id in targets:
                    try:
                        ...
                        if pid > last_id:
                            last_id = pid
                            save_last_freelanceru_project(last_id)
                            G_LAST_ID.set(last_id)
                        try:
            ...
            M_ERRS.inc()
            log.exception("Unhandled error in Freelanceru loop: %s", e)
            await asyncio.sleep(3)

    log.info("Freelanceru parser stopped by signal")
